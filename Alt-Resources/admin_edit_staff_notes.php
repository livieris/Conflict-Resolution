		...
		<div id="body">
			...
			<div class="section">
				...
				<div class="floatCenterText">
					<h2>
					<?php
						echo "Admin: ".$_SESSION['searchResultFirstName'] . " " . $_SESSION['searchResultLastName'] . " Staff Note"; 
					?>
					</h2>
					<?php
					if ($_SERVER['REQUEST_METHOD'] == "POST"){
						$note = $_POST['editStaffNotes'];
						$query = $db->prepare("UPDATE notes SET staffNote = ? WHERE userName = ?;");	
						$query->execute(array($note, $_SESSION['searchUserName']));
						
						echo("<div class='floatCenterText'><h3>Staff note changed</h3></div>");
					}
					?>
				</div>
			</div>
			...
			<div id="loginDiv">
				...
				<!-- Here will print out from db and then submit changes-->
					<form class ="contactInfo" action="admin_edit_staff_notes.php" method="post">
						<textarea class="noteBox" name="editStaffNotes">
						<?php
					...
					<div class="floatRightText">
						<p>
							<a id="button" href="searchResultProfile.php">Back</a>
						</p>
						<br />
					</div>
